      ...
      <section className="bg-gray-50 p-3 antialiased dark:bg-gray-900 sm:p-5">
        <div className="mx-auto max-w-screen-2xl px-4 lg:px-12">
          ...
          <Grid<ContributorDto>
            ...
            data={contributors}
            // passing key's to each of the array elements to prevent key mismatch. if the keys don't match, we can get hydration errors'
            cells={(contributor) => [
              <div key={contributor.id}>{contributor.name}</div>,
              <div
                key={contributor.id}
                className="flex w-full items-center gap-x-2 font-medium text-gray-900 dark:text-white"
              >
                <Avatar
              ...
              <div key={contributor.id}>{contributor.title}</div>,
              // trying to differentiate between full time workers and the rest of the catergories with a blue background.
              <div key={contributor.id}>
                {contributor.category === "Full" ? (
                  <Badge key={contributor.id} color="blue">
                    {contributor.category}
                  </Badge>
                ) : (
                  <Badge key={contributor.id} color="light">
                    {contributor.category}
                  </Badge>
                )}
              </div>,
              <div key={contributor.id}>{contributor.payments}</div>,
              <div key={contributor.id} className="font-semibold text-blue-600">
